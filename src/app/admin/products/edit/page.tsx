    ...
    const fetchProduct = async () => {
      try {
        ...
        if (data) {
          ...
          setEnableSizes(isSizeEnabled);
          if (dynamicSizes) {
            if (dynamicSizes["6x7"]) {
              setSize6x7({
                enabled: !!dynamicSizes["6x7"].enabled,
                price: dynamicSizes["6x7"].price?.toString() || '',
                originalPrice: dynamicSizes["6x7"].originalPrice?.toString() || ''
              });
              setSize6x7Image(dynamicSizes["6x7"].image || '');
            }
            if (dynamicSizes["7x8"]) {
              setSize7x8({
                enabled: !!dynamicSizes["7x8"].enabled,
                price: dynamicSizes["7x8"].price?.toString() || '',
                originalPrice: dynamicSizes["7x8"].originalPrice?.toString() || ''
              });
              setSize7x8Image(dynamicSizes["7x8"].image || '');
            }
          }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (enableSizes) {
        ...
        defaultOriginalPrice = size6x7.enabled 
    ...
    return (
      <AdminLayout>
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px' }}>
          ...
          <p style={{ marginLeft: '12px', color: 'var(--text-light)' }}>Loading product details...</p>
        </div>
      </AdminLayout>
    );
  }
    ...
    <AdminLayout>
      <div className={styles.sectionHeader}>
        <div>
          ...
          <p className={styles.pageSubtitle}>Update product listing details.</p>
        </div>
        <div className={`${styles.headerActions} ${styles.hideOnMobile}`}>
          ...
          <button className={styles.primaryBtn} onClick={handleSave} disabled={loading || uploadingIndex !== null}>
            ...
            <span>{loading ? 'Saving...' : uploadingIndex !== null ? 'Uploading...' : 'Save Changes'}</span>
          </button>
        </div>
      </div>
  ...
  return (
    <Suspense fallback={
      <AdminLayout>
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px' }}>
          <div className={styles.loader}></div>
          <p style={{ marginLeft: '12px', color: 'var(--text-light)' }}>Loading product editor...</p>
        </div>
      </AdminLayout>
    }>
      <EditProductForm />
